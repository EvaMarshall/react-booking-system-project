...
import { createBooking } from "../apis/booking"; //  Booking API integration
...
function GuestBookingOverview() {
    const location = useLocation();    // Retrieves selected booking details passed via navigation state
    const navigate = useNavigate();
    const { selectedRange, totalPrice, guestInfo, selectedCottage } = location.state || {};
    const [showConfirmation, setShowConfirmation] = useState(false);//  Controls confirmation pop-up visibility
    const [isConfirmed, setIsConfirmed] = useState(false);    //  Tracks whether the user has confirmed booking details

    const lastNight = new Date(selectedRange[selectedRange.length - 1]);    // Calculates check-out date by adding one day to the last selected night
    const checkOutDate = new Date(lastNight);    // This avoids off-by-one errors and aligns with backend blocking logic
    checkOutDate.setDate(checkOutDate.getDate() + 1); //  Visual aid: check-out is the day after last night
    const formattedCheckOut = checkOutDate.toISOString().split("T")[0]; //  Ensures backend receives 'YYYY-MM-DD'
    ...
    const handleConfirmBooking = async () => {
        console.log("Booking confirmation triggered"); // Debug log
        ...
        const bookingPayload = {
            ...
            check_in_date: selectedRange[0], //  First selected date
            check_out_date: formattedCheckOut, //  One day after last selected night
        ...
        try {
            ...
            console.error("Booking creation failed:", err); //  Captures 503 or validation errors
            // TODO: Optionally show error feedback to user
        }
    };
        ...
        <div className="min-h-screen flex flex-col">
            ...
            <main className="flex flex-col items-center justify-center flex-grow p-6">
                ...
                <div className="mt-6 flex space-x-4 w-full max-w-sm">
                    ...
                    <button
                        className={`px-6 py-3 rounded-lg text-lg ${!isConfirmed ? "bg-gray-400 cursor-not-allowed" : "bg-green-600 text-white"
                            } w-full`}
